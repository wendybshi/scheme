...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError('length of values is not the same as length of arguements')
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        arguments=operands.map(lambda opr: scheme_eval(opr, env))
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Type Error: {0}'.format(self))
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        b=bindings.first
        f=Pair(b.first,f)
        v=Pair(scheme_eval(b.second.first,env), v)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            result=scheme_apply(scheme_eval(first,env),rest.map(lambda opr: scheme_eval(opr, env)), env)
            # END PROBLEM 20
    return result
